...
def boot_system():
    print(f"{GREEN}QW Kernel Version 0.1.5")
...
def run_fetch():
    """Neofetch benzeri sistem bilgi ekranı"""
...
def get_weather(city):
    """wttr.in üzerinden hava durumu bilgisini çeker"""
    target = city if city else ""
    print(f"{YELLOW}Hava durumu verisi çekiliyor...{RESET}")
    try:
        # Terminal çıktısı için format '0' (Sadece o anki hava durumu)
        ...
        print(f"{RED}Bağlantı hatası: Hava durumu alınamadı. ({e}){RESET}")

def run_matrix_effect():
    """Terminalde yeşil Matrix dijital yağmur efekti başlatır."""
    print(f"{GREEN}Matrix moduna giriliyor... Çıkmak için Ctrl+C tuşlayın.{RESET}")
    time.sleep(1)
    ...
    try:
        ...
        print(f"\n{CYAN}Matrix modundan çıkıldı.{RESET}")
        time.sleep(0.5)

def print_tree(dir_path, prefix=""):
    """Klasör hiyerarşisini ağaç yapısı şeklinde gösterir."""
    try:
        ...
        print(f"{prefix}└── {RED}[Erişim Engellendi]{RESET}")
    except Exception as e:
